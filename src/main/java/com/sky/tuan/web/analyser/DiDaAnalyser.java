...
@Component
public class DiDaAnalyser extends BaseAnalyser implements IAnalyser {
    
	@Value("#{didaSetting['dida.cityUrl']}")
	private String cityUrl;
	
	@Value("#{didaSetting['dida.key']}")
	private String key;
	
	@Value("#{didaSetting['dida.dealUrl']}")
	...
	static Logger logger = Logger.getLogger(DiDaAnalyser.class.getName());
	...
	public void analyse() throws IOException
	{
		...
		for(Element eCity:eCitys)
		{
           ...
           for(Element url :urls)
           {
        	   ...
        	   int startIndex=loc.indexOf("id=")+3;
        	   
        	   Deal deal=getDealRepository().findByKey(this.key+loc.substring(startIndex));
        	   ...
        	   if(deal==null)
        	   {
        		   deal=new Deal();
        		   isAddData=true;
        		   deal.setDealType(TuanTypeEnum.Dida.getValue());
        	   }
               ...
               deal.setKey(this.key+loc.substring(startIndex));
