...
@Component
public class DianPingAnalyser extends BaseAnalyser implements IAnalyser {
    
	@Value("#{dianpingSetting['dianping.cityUrl']}")
	private String cityUrl;
	
	@Value("#{dianpingSetting['dianping.key']}")
	private String key;
	
	@Value("#{dianpingSetting['dianping.dealUrl']}")
	...
	static Logger logger = Logger.getLogger(DianPingAnalyser.class.getName());
	...
	public void analyse() throws IOException
	{
		...
		Elements eCitys = cityDoc.select("city");
		...
		for(Element eCity:eCitys)
		{
           ...
           try
           {
        	   dealDoc = Jsoup.connect(dealUrl+"?cityId="+cityId)
        		   .timeout(60000)
        		   .maxBodySize(Integer.MAX_VALUE)
        		   .ignoreContentType(true)
        		   .parser(new Parser(new XmlTreeBuilder()))
           ...
           for(Element url :urls)
           {
        	   ...
        	   if(deal==null)
        	   {
        		   deal=new Deal();
        		   isAddData=true;
        		   deal.setDealType(TuanTypeEnum.Dianping.getValue());
        	   }
               ...
               deal.setImageUrl(display.getElementsByTag("bigimage").first().text());
               ...
               if(deal.getOriginalPrice()>0)
               {
                  deal.setRebate(deal.getCurrentPrice()/deal.getOriginalPrice());
               }
               else
               {
            	   deal.setRebate(0);
               ...
               deal.setShortTitle(display.getElementsByTag("abstract").first().text());
               ...
               splitTags(tags,display.getElementsByTag("category").first().text());
               
               splitTags(tags,display.getElementsByTag("subcategory").first().text());
